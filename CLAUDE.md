...
**Mise en scène de la capture — le « projecteur »** : à chaque information captée, le panneau de droite s'éteint à **16 %** sauf le champ concerné (**maintien borné en temps réel**, fondu 200 ms, halo mint). Quatre variantes : le champ (`spotlight`), la **validation EDI** (`spot-edi`, qui garde le score allumé pour que « lead chaud » et validation se lisent ensemble), le **score** (`spot-score`) et le **bandeau d'envoi** (`spot-sent`). C'est *la* démonstration de l'outil — l'œil part à droite sans commentaire. La respiration occupe un temps mort du chat, donc aucune phrase n'est coupée.

**Le projecteur est tenu par la boucle du scénario, jamais par un `setTimeout`** (01/09) : `holdSpot()` passe par `wait()` → `gate()`, donc **le halo reste allumé tant qu'on est en pause**. C'est tout l'intérêt — l'instant où on veut s'arrêter pour dire « regardez : autonomie captée, sans formulaire » est exactement celui où un minuteur indépendant reprenait la main au bout de 450 ms. Le minuteur subsiste uniquement pour le **mode Live**, qui remplit le dossier hors de la boucle. Corollaire : le défilement du panneau est **instantané** (`behavior:"instant"`) — un défilement doux dure aussi longtemps que le projecteur, le halo pouvait donc s'éteindre pendant que le champ arrivait.

**Le maintien est borné en temps réel, pas proportionnel** (01/09) : `SPOT_HOLD = min(1400, max(520, 900/pace))`. Le seul temps réellement lisible est le **plateau** = maintien − 157 ms de fondu d'entrée ; à 450 ms il ne restait que 294 ms à 1× et **68 ms à 2×**, moins qu'une saccade oculaire — le champ s'éteignait avant que l'œil n'arrive. Le plateau passe à ~743 ms à 1×. ⚠️ `holdSpot()` passe par `wait()`, qui divise déjà par `pace` : il faut pré-multiplier (`wait(SPOT_HOLD() * pace())`), sinon le maintien redevient proportionnel. Coût : scénario 121 → **126 s** à 1×.

**`scrollIntoView` ne connaît ni le bandeau collant ni le halo** (01/09) — deux régressions du même jour, trouvées à la mesure : (1) il cale la cible **au pixel** sur le bord bas, donc l'ombre portée de 23 px du projecteur était rognée à 98 % et la carte semblait « mangée par le cadre » ; (2) en **remontée** il cale la cible en haut du scrollport, c'est-à-dire **sous le bandeau collant opaque** — le champ « La personne âgée » était allumé par le projecteur à **0 % de visibilité**, à toutes les hauteurs de fenêtre. Corrigé par une seule règle : `.lead-side { scroll-padding-block: 67px 28px }` (67 ≥ hauteur de `.lead-progline`, 55 px mesurés).

**Le pli du dossier — `snapPanel()`** (01/09) : le bord bas de la zone visible ne doit jamais tomber au milieu d'une carte. Après chaque défilement, on ramène le pli dans une gouttière, sans faire sortir la carte visée ni la laisser passer sous le bandeau. ⚠️ Piège : un candidat **borné** à 0 ou au maximum revient au point de départ — il gagne le tri par proximité (distance nulle) sans rien recadrer ; il faut revérifier qu'il supprime réellement la coupure. Mesuré sur le run complet, à 620/745/900 px de haut : **132/359/434 → 35/30/31** frames avec un bloc tranché.
⚠️ **Le bloc EDI grandit de 22 px APRÈS son unique `scrollIntoView`** (le compteur « n/6 » apparaît, un nom passe sur 2 lignes quand « ✗ non retenue » s'affiche) : il débordait de **14 px** au pic « 2/6 validées », bordure et halo coupés, dernière ligne plaquée au bord — pendant 2,4 s à 1× et **5,6 s à 0,4×**. `showEdi` rappelle donc `snapPanel` **après chaque ligne résolue** : le cadre a désormais 6 px de dégagement et la dernière ligne 21 px.
⚠️ **Écarté après mesure : le masque de dégradé** (`mask-image` sur `.lead-side`). Il s'applique à l'élément entier, **fond compris** : le panneau vert virait au blanc laiteux sur ses 28 derniers pixels. Un voile collant de la couleur du panneau fonctionne, lui, mais devient **inutile une fois `snapPanel` correct** (35/30/31 → 38/33/33, écart nul) et il estompait la dernière ligne EDI alors qu'elle n'était plus coupée — retiré.

**Les trois preuves sont cadrées de force** (01/09) : `showScore()` et `showSent()` amènent leur bloc à l'écran (`scrollIntoView` sur le **panneau**, pas sur le chat — `down()` ne scrollait que la conversation) puis le tiennent sous projecteur, 900 ms pour le score, 1 200 ms pour l'envoi. Le badge « 🔥 Chaud · 9/10 » et le bandeau « lead envoyé » ne peuvent plus naître sous la ligne de flottaison, quelle que soit la hauteur du poste.

**Ligne de progression collante + compteur `n / 11`** (01/09) : la barre seule ne dit pas « 7 sur 11 » à trois mètres en visio. ⚠️ Deux pièges : (1) le padding haut de `.lead-side` passe sur `.lead-head`, sinon le bloc collant se cale 24 px trop bas et le dossier défile visiblement au-dessus de lui ; (2) le projecteur atténue le **contenu** du bandeau (`.lead-prog`, `.lead-count`), **pas son conteneur** — à 16 % d'opacité un fond opaque cesse de masquer ce qui défile dessous. En RTL, `direction: ltr` sur le compteur (même parade que celui de la console).

**2 modes (toggle dans la top-bar) :**
- **Démo scriptée (déterministe)** — *mode par défaut, démarrage manuel* : scénario figé (**texte intégral validé le 10/06**, accueil refondu le 28/07), streaming mot-à-mot, **vitesse 0,4×–2×, défaut 1×**, layout **face-à-face plein écran** (hauteur auto-adaptative, chat et dossier défilent chacun en interne). **Zéro API pour le texte.** Durée : **126 s à 1× voix coupée**, climax « lead envoyé » à **119 s**. ⚠️ Le défaut était **0,4×** depuis le 09/06 (commit `f6ce715`), soit ~5 min de scénario — décision d'avant la voix et la console, revenue à 1× le 01/09.
...
⚠️ **Le récapitulatif e-mail de la démo est construit depuis le dossier**, il n'est plus écrit à l'avance (01/09) : la ligne « contact » lit le champ `consent` affiché à l'écran. Avant, elle mentionnait « `07 57 99 11 40` si non-consentement » alors que le scénario fait dire « oui » à Sylvie — une contradiction visible, sur le point métier précis qu'un partenaire attentif regarde.

**Ordre de fin (01/09)** : l'envoi du lead suit **immédiatement** la validation EDI (`4/6 validées` → « lead envoyé »), et les quatre cartes famille (PDF, promo, vidéo, itinéraire) se jouent **après**, pendant que le bandeau reste affiché. Le climax partenaire n'attend plus des livrables qui s'adressent à la famille. ⚠️ Le gain n'est que de **7 s voix coupée** (mesuré) — il se compte surtout **avec la voix**, les deux répliques d'Emma de la fin étant lues intégralement.
...
- **Argumentaire Cap Retraite (Bernard)** : `knowledge/pitch-cap-retraite.md` — **réécrit le 29/07** sur l'axe BinAI Leads. Antisèche d'une page pour la visio : `knowledge/antiseche-rdv.md`. Pistes de canaux (note interne, pas un argumentaire) : `knowledge/acquisition.md`.
...
### 🎯 L'axe du pitch — « BinAI Leads – génération de prospects » (arrêté le 28/07, nommé le 06/08)

> **Ce qu'est BinAI Leads (précisé le 29/07, nommé le 06/08)** — ni un renommage d'Emma, ni une offre faite à Cap Retraite : c'est **le procédé de création de leads par conversation**, éprouvé sur l'EHPAD avec Emma et **duplicable à d'autres secteurs**. Emma reste Emma côté famille. **BinAI Leads reste la propriété de BinAI.**
