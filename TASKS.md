...
- **Réglages après première relecture** : (1) respiration après capture raccourcie — projecteur 900 → **640 ms**, attente 1 000 → **700 ms** (scénario 125 → **121 s**) ; (2) les deux boutons de la console affichaient « Reprendre » (le principal en pause + le secondaire) → le secondaire redevient **« Rejouer »** ; (3) **projecteur sur la validation EDI** (`.spot-edi`) : tout s'éteint sauf le bloc EDI **et le score d'urgence**, pour que « lead chaud » et validation partenaire se lisent d'un seul regard. ⚠️ Au passage : `fastForward()` ne rejouait pas les steps `score`, donc un saut au chapitre 8 arrivait avec un score éteint — `showScore()` scindé en `applyScore()` (état seul) + animation.
